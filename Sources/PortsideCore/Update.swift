...
public struct Release: Decodable, Sendable {
    ...
    /// The app zip: `Portside.zip` or `Portside-0.2.0.zip`.
...
public enum UpdateError: LocalizedError, Equatable {
    ...
    public var errorDescription: String? {
        switch self {
        case .noAppInArchive: "The download doesn't contain an app."
        ...
        case .invalidSignature(let detail): "The downloaded app's code signature is invalid: \(detail)"
        case .command(let cmd, let status): "\(cmd) failed (exit \(status))."
        }
    }
}
...
public enum UpdateInstaller {
    ...
    /// (restoring the old copy if that fails) and relaunches it. Run it detached, then quit.
    public static func swapScript(pid: Int32, newApp: URL, destination: URL, relaunch: Bool = true) -> String {
        ...
        rm -rf \(quote(backup.path))
        \(relaunch ? "open \(quote(destination.path))" : "")
        """
    }
